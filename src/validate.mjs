...
// Both directories are resolved from the invocation, not from where this file happens to sit:
// the validator is a package and the specification it validates against is a separate checkout.
// The defaults are what an oold-schema checkout looks like, so running it from that repo's root
// needs no arguments and behaves as it did when the script lived there.
...
if (metaArg === undefined) {
  console.error("--meta needs a directory holding the OO-LD meta-schemas");
  process.exit(2);
}
const exDir = resolve(process.cwd(), positional[0] ?? "examples");
const metaDir = resolve(process.cwd(), metaArg ?? "meta");
const readMeta = (name) => {
  try {
    return JSON.parse(readFileSync(join(metaDir, name), "utf8"));
  } catch (err) {
    console.error(`could not read ${name} from ${metaDir}: ${err.message}`);
    console.error("Pass --meta <dir> pointing at a checkout of OO-LD/oold-schema's meta/ folder.");
    process.exit(2);
  }
};
const meta = readMeta("oold-meta-schema.json");
...
const baseMeta = readMeta("oold-meta-schema-base.json");
const uiMeta = readMeta("oold-ui-meta-schema.json");
const patternLint = readMeta("oold-pattern-lint.schema.json");
...
const ruleCatalogDoc = (() => {
  try { return JSON.parse(readFileSync(join(metaDir, "oold-rules.json"), "utf8")); }
  catch { return null; }
})();
...
const ruleCatalogSchema = (() => {
  try { return JSON.parse(readFileSync(join(metaDir, "oold-rules.schema.json"), "utf8")); }
  catch { return null; }
})();
